...
def download_voice_message(message):
    ...
    if (global_user_ready_to_signin):
        global_user_ready_to_signin=False
        downland_audio(message,"user_voice_from_chat")
        bot.send_message(message.chat.id,"We got your voice")
    if (global_user_ready_to_recognition):
        global_user_ready_to_recognition=False
        downland_audio(message,"user_voice_from_chat_for_recognition")
        pathToAudioFile_usedDuringRegistration = f'user_voice_from_chat{message.chat.id}.wav'
        pathToAudioFile_usedForRecognition = f'user_voice_from_chat_for_recognition{message.chat.id}.wav'
...
def downland_audio(message,fileName):
    file_info = bot.get_file(message.voice.file_id)
    file = requests.get('https://api.telegram.org/file/bot{0}/{1}'.format(token, file_info.file_path))
    with open(fileName + f'{message.chat.id}.ogg', 'wb') as f:
        f.write(file.content)
        f.close()
    data, samplerate = sf.read(fileName + f'{message.chat.id}.ogg')
    sf.write(fileName + f'{message.chat.id}' +".wav",data, samplerate)
    os.remove(fileName + f'{message.chat.id}.ogg')
    #os.remove(fileName + f'{message.chat.id}.ogg')
